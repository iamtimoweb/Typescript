...
import { addUID, add_with_name_property } from "./generics/generics.js";
import { greetPerson } from "./interfaces/isPerson.js";
const myName = "Mario";
console.log(myName);
 ...
 * Explicit Types
 ******************************/
let character; // will contain only string type
let experience; // will contain only numbers
let isDisabled; // will contain only boolean types
// arrays
const cities = [];
cities.push("kampala");
// union types
const uid = 0;
const mixed_types = []; // can contain string, number, boolean types
 ...
 *******************************/
let animal; // can take up any type of value ie strings, numbers, objects etc
animal = "lion";
animal = 56;
// arrays
let hotels = []; // different value types can be pushed to the array after it has been declared.
...
// object:
let countries; // the first two keys can take up any value types and the last key takes up strictly a value of string type.
...
const difference = (a, b, c = 0) => console.log(`The difference is ${a + b - c}`);
difference(20, 50, 15);
...
total(20, 10);
const logDetails = (uuid, password) => console.log(`The password has a uuid of ${uuid}`);
...
const person1 = {
    name: "Mark Bugembe",
    age: 50,
    speak: (language) => console.log(`I speak ${language}`),
    spend: (amount) => amount,
};
greetPerson(person1);
/*************************
